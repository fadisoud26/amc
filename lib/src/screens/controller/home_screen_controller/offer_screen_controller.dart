...
import 'package:alwadi_medical_center/src/screens/model/offer_screen_model.dart';
...
class OfferScreenController extends GetxController {
  ...
  void offerAPI() {
    http.get(APIURI.offerURI, headers: headerValue()).then((value) => {
          if (jsonDecode(value.body)[ApiParName.result] == 'ok')
            {
              offerModel = OfferModel.fromJson(jsonDecode(value.body)),
